...
public class BusinessMethods implements IBuisnessMethods {
    ...
    public List<String> getPfIdsFor(List<Commande> commandes) {
        return CommandeOptics.listT.andThen(Commande.commande2chassisTronconT).all(commandes).map(ChassisTroncon::pfId).toList();
    }

    @Override
    public List<String> getPfIdsForWhere(List<Commande> commandes, Predicate<ChassisTroncon> where) {
        return CommandeOptics.listT.andThen(Commande.commande2chassisTronconT).filter(where).all(commandes).map(ChassisTroncon::pfId).toList();
    }

    public List<String> getPfIdsFor(Commande commande) {
        return Commande.commande2chassisTronconT.all(commande).map(ChassisTroncon::pfId).toList();
    }

    @Override
    public void doSomething(Commande commande) {
        Commande.commande2chassisTronconT.all(commande).forEach(
                ChassisTroncon::doSomething);

    }

    public void doSomethingForList(List<Commande> commandes) {
        CommandeOptics.listT.andThen(Commande.commande2chassisTronconT).all(commandes).forEach(
                ChassisTroncon::doSomething);
    ...
    public void doSomethingWhere(List<Commande> commandes, Predicate<ChassisTroncon> where) {
        CommandeOptics.listT.andThen(Commande.commande2chassisTronconT).filter(where).all(commandes).forEach(
                ChassisTroncon::doSomething);
    }
